      ...
      <Transition timeout={transitionTime} in={!collapsed}>
        {state => (
          <div
            ...
            <div
              className={`${
                isCollapseVisible ? 'flex' : 'dn'
              } absolute bottom-0 pointer-events-none w-100 h-100 flex-column justify-end`}>
              <div
                style={transitionStyle(fadeOutTime)}
                className={`${
                  state === 'entered' ? 'o-0' : ''
                } fade-bottom w-100 h-50`}
              />
              <div
                className={`${
                  state === 'entered' ? 'bg-transparent' : 'bg-base'
                } tc w-100 pointer-events-auto`}>
                <div
